...
0 2 * * * /path/to/root/CA/redis.sh prod backup

# Cleanup semanal
0 3 * * 0 /path/to/root/CA/redis.sh prod clean
```
...
```bash
# Status check
*/5 * * * * /path/to/root/CA/redis.sh prod health

# Alertas via webhook
#!/bin/bash
if ! /path/to/root/CA/redis.sh prod health > /dev/null 2>&1; then
